...
PDokanFCB GetOrCreateUninitializedFcb(__in PREQUEST_CONTEXT RequestContext,
  ...
  if (!RequestContext->Vcb->Dcb->StoreFcbInAvlTable) {
    ...
    for (thisEntry = listHead->Flink; thisEntry != listHead;
      ...
      DOKAN_LOG_FINE_IRP(RequestContext,
                         "Has entry FCB=%p FileName=\"%wZ\" FileCount: %lu",
                         fcb, &fcb->FileName, fcb->FileCount);
      if (fcb->FileName.Length == FileName->Length  // FileNameLength in bytes
          && RtlEqualUnicodeString(FileName, &fcb->FileName,
  ...
  if (!RequestContext->Vcb->Dcb->StoreFcbInAvlTable) {
    *NewElement = TRUE;
    InsertTailList(&RequestContext->Vcb->NextFCB, &fcb->NextFCB);
    return fcb;
  }
...
PDokanFCB DokanGetFCB(__in PREQUEST_CONTEXT RequestContext,
  ...
  if (newElement) {
    ...
    if (!DokanInitializeFcb(RequestContext, fcb)) {
      if (RequestContext->Vcb->Dcb->StoreFcbInAvlTable) {
        BOOLEAN removed = RtlDeleteElementGenericTableAvl(
            &RequestContext->Vcb->FcbTable, &fcb);
        ASSERT(removed);
...
RTL_GENERIC_COMPARE_RESULTS DokanCompareFcb(__in struct _RTL_AVL_TABLE *Table,
  ...
  LONG result =
      RtlCompareUnicodeString(&firstFcb->FileName, &secondFcb->FileName,
                              /*CaseInSensitive=*/TRUE);
